    ...
    with graph.as_default():
        ...
        with tf.variable_scope('nnlm' + 'embedding'):
            embeddings = tf.Variable(tf.random_uniform(
                [args.vocab_size, args.word_dim], -1.0, 1.0))
        ...
        def infer_output(input_data):
            ...
            # print(input_data_emb.shape)  120 * 5 * 50
            # time.sleep(2)
            input_data_emb = tf.reshape(
                input_data_emb, [-1, args.win_size * args.word_dim])
            hidden = tf.tanh(tf.matmul(input_data_emb, weight_h)) + b_1
            hidden_output = tf.matmul(
                hidden, softmax_u) + tf.matmul(input_data_emb, softmax_w) + b_2
            output = tf.nn.softmax(hidden_output)
            return output

        outputs = infer_output(input_data)
        one_hot_targets = tf.one_hot(tf.squeeze(
            targets), args.vocab_size, 1.0, 0.0)  # 120 * 4557
        print((tf.log(outputs)).shape)
        print((tf.log(outputs) * one_hot_targets).shape) # 120 * 4557
        print(tf.reduce_sum(tf.log(outputs) * one_hot_targets, 1).shape) # 120
        time.sleep(1111)
        loss = - \
            tf.reduce_mean(tf.reduce_sum(tf.log(outputs) * one_hot_targets, 1))
        # Clip grad.
        optimizer = tf.train.AdagradOptimizer(0.1)
        gvs = optimizer.compute_gradients(loss)
        capped_gvs = [(tf.clip_by_value(grad, -args.grad_clip,
                                        args.grad_clip), var) for grad, var in gvs]
        optimizer = optimizer.apply_gradients(capped_gvs)

        embeddings_norm = tf.sqrt(tf.reduce_sum(
